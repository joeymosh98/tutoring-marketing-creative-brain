...
description: |
  Tutero brand identity skill. Use this skill whenever creating ANY output for Tutero -documents, presentations, briefs, reports, social content, emails, ads, or any other asset. This includes Word docs (.docx), PowerPoint decks (.pptx), PDFs, HTML pages, React components, or any written content. Must be consulted before writing a single line of code or content for any Tutero deliverable. Contains the sub-brand decision rule (purple = core/home, blue = tutoring), exact brand colours for both palettes, typography, four logo variants, tone of voice, product naming conventions, imagery guidance, the Tutoring Ad Aesthetic recipe (gradient hero title, white stroke, light blue card, UGC photo, green check pills) for parent-facing tutoring marketing only, and ready-to-use code constants for docx-js and pptxgenjs.
cssclasses:
Status: 🟡
tags:
...
SKILL_DIR = '/mnt/skills/user/visual-identity'
...
The single most recognisable element of tutoring marketing is the **chunky gradient hero title**. It's the thing that makes a Tutero tutoring ad recognisable at a glance in the feed.

**Recipe:**
- **Font:** Satoshi (or Arial fallback in docs), **Black / Heaviest weight**, large display size
- **Fill:** the **tutoring gradient** applied left-to-right across the word(s). The gradient provides enough visual impact on its own.
- **No white stroke.** Production testing confirmed that `-webkit-text-stroke` makes the text look bloated and is not part of the winning aesthetic. Never add `text-stroke` or `paint-order: stroke fill` to gradient hero text.
- **Two gradient variants** -use the right one for the background:
  - **On light-blue backgrounds:** `#1D49E3 0%` → `#00A3FF 15%` → `#66A693 35%` → `#E8A909 60%` → `#FF7A00 85%`
  - **On photo/dark backgrounds:** `#5B8CFF 0%` → `#33C8FF 15%` → `#7EC9A8 35%` → `#F5C542 60%` → `#FF8C33 85%` (same hues, lifted ~30% brighter for readability)
- **Gradient stops end at 85%**, not 100%. This ensures the full blue-to-orange spectrum completes within the visible text area. At 100%, the orange falls outside the text and is invisible.
...
- **No stroke** -plain solid colour only. The subhead should be calm and scannable, not competing with the gradient hero.
- **Format:** lowercase `for Prep – Year 12 🇦🇺` (one flag, right side)
...
- **Footer line** -always just `tutero.com.au`, no extra copy. Keep it clean.
...
- [ ] Hero noun has the **left-to-right tutoring gradient fill** -no white stroke
- [ ] Correct gradient variant used: standard on light-blue backgrounds, brighter on photo/dark
- [ ] Subhead ("for Prep – Year 12 🇦🇺") is solid blue, no stroke, lowercase
...
```css
/* CSS -tutoring gradient on light-blue (#F0FAFF) backgrounds */
...
color: transparent;
/* NO -webkit-text-stroke -production testing confirmed stroke looks bloated */
font-family: "Satoshi", "Arial", sans-serif;
font-weight: 900;
line-height: 1.25; /* minimum -prevents descender clipping on 'g', 'y' etc */
```

```css
/* CSS -tutoring gradient on photo/dark backgrounds (brighter variant) */
background: linear-gradient(
  90deg,
  #5B8CFF 0%,
  #33C8FF 15%,
...
```svg
<!-- SVG -gradient definition for hero title fills (light-blue background variant) -->
<defs>
  <linearGradient id="tutoringGradient" x1="0%" y1="0%" x2="100%" y2="0%">
    <stop offset="0%"   stop-color="#1D49E3"/>
    <stop offset="15%"  stop-color="#00A3FF"/>
    <stop offset="35%"  stop-color="#66A693"/>
    <stop offset="60%"  stop-color="#E8A909"/>
    <stop offset="85%"  stop-color="#FF7A00"/>
  </linearGradient>
</defs>
<!-- NO stroke attribute -gradient fills without outline -->
<text fill="url(#tutoringGradient)" font-family="Satoshi" font-weight="900">
  Online Tutoring
</text>
```
